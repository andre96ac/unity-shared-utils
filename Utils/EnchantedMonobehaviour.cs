...
public class EnchantedMonobehaviour : MonoBehaviour
{
    ...
    protected void DelayExecutionSeconds(float timeToWait, Action actionToRun, bool realTime = false){
        StartCoroutine(delayExecutionSecondsCorutine(timeToWait, actionToRun, realTime));
    }

    private IEnumerator delayExecutionSecondsCorutine(float timeToWait, Action actionToRun, bool realTime){
        if(realTime){
    ...
    protected void DelayExecutionEndFrame(Action actionToRun){
        StartCoroutine(delayExecutionEndFrameCorutine(actionToRun));
    }

    private IEnumerator delayExecutionEndFrameCorutine(Action actionToRun){
        yield return new WaitForEndOfFrame();
        actionToRun();
    }
    protected void DelayExecutionFixedUpdate(Action actionToRun){
        StartCoroutine(delayExecutionFixedUpdateCorutine(actionToRun));
    }

    private IEnumerator delayExecutionFixedUpdateCorutine(Action actionToRun){
        yield return new WaitForEndOfFrame();
        actionToRun();
    }
